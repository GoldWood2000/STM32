...
//舵机PWM值最大值与最小值
int Servo_max=2200, Servo_min=900;
...
{   
	  ...
	  float amplitude=3.5;
		...
		else if(Car_Mode==2||Car_Mode==3) //TOP_AKM_BS - 顶配阿克曼小车摆式悬挂
		{
			...
			// Front wheel steering Angle limit (front wheel steering Angle controlled by steering engine), unit: rad
			//前轮转向角度限幅(舵机控制前轮转向角度)，单位：rad
			AngleR=target_limit_float(AngleR,-0.47,0.34);
      ...
      //Inverse kinematics //运动学逆解
			...
			else 
			{
				...
				MOTOR_B.Target = Vx;     //计算出右轮的目标速度
			}
			Slide_target= -2450*pow(AngleR, 3) + 1290*pow(AngleR, 2) + 3242*AngleR - 228.6;
		...
		else if(Car_Mode==4||Car_Mode==5) //TOP_AKM_DL - 顶配阿克曼小车独立悬挂
		{
			...
			// Front wheel steering Angle limit (front wheel steering Angle controlled by steering engine), unit: rad
			//前轮转向角度限幅(舵机控制前轮转向角度)，单位：rad
			AngleR=target_limit_float(AngleR, -0.48, 0.32);
      ...
      Slide_target= 4515*pow(AngleR, 3) - 174.2*pow(AngleR, 2) - 4323*AngleR + 42.01;
			Slide_target= Slide_target + SERVO_BIAS;		
...
{
	 static float Bias,Pwm=1500,Last_bias;
	 float kp=0.004, kd=0.009;
	 ...
	 if(Car_Mode==2||Car_Mode==3)
	 {
	   ...
	   if(SlideTarget<0||SlidePosition<-200) Pwm+=kp*1.6*(Bias-Last_bias)+kd*1.6*Bias; 
	   else Pwm+=kp*(Bias-Last_bias)+kd*Bias;   
	 }
...
{
	float Encoder_A_pr,Encoder_B_pr;   	//获取编码器的原始数据
	Encoder_A_pr= Read_Encoder(2);  
	Encoder_B_pr=-Read_Encoder(3);

	MOTOR_A.Encoder= Encoder_A_pr*CONTROL_FREQUENCY/Encoder_precision*Wheel_perimeter; //编码器原始数据转换为车轮速度，单位m/s
